...
import com.supersports.sportsflashes.common.utils.AppConstant.DateTime.DATE_TIME_FORMAT_LOCAL
...
//        String selecteTime=hour12format_local+":"+ minute1+" "+ampmStr;
    ...
    fun getTimeDiffrence(date1S: String, date2S: String): String {
        val date1 = SimpleDateFormat(AppConstant.DateTime.DATE_TIME_FORMAT_ISO).parse(date1S)
        val date2 = SimpleDateFormat(AppConstant.DateTime.DATE_TIME_FORMAT_ISO).parse(date2S)
    ...
    fun calculateTimeBetweenTwoDates(formatString: String, date1s: String, date2s: String): String {
        val format = SimpleDateFormat(formatString)
        if(date1s.isEmpty() || date2s.isEmpty()){
            return ""
        }
        val date1 = format.parse(date1s)
        val date2 = format.parse(date2s)
